...
| M2a ✅ | 方向场 M2a ② / Stage 2 M2a 规则式生成器 ③ / param_map ④ / 经典分割 / 规则式 Stage 3 / 后端切换 / 权重下载脚本 ⑥ | 真实规则式全链路 CPU 可跑（39 测试通过）；参数映射表落地 |
| M2 | ① SAM + DINOv2 接入（ViT-B 档）⑤ 快路径移入 worker-gpu ⑦ CLA 强制 | 快路径 P99 < 1s（T4）；方向场 R1 门禁报告 |
